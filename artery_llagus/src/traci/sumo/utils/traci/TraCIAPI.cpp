...
//~ make vehicles go slower so ambulance can go through
//------------------------------------------------------
void
TraCIAPI::VehicleScope::my_slowDown(const std::string& vehicleID, int during_ms) const {
    auto my_speed = getSpeed(vehicleID);
    my_speed /= 5;
    slowDown(vehicleID, my_speed, during_ms);
}

double
TraCIAPI::VehicleScope::my_getSpeed(const std::string& vehicleID) const {
    return myParent.getDouble(libsumo::CMD_GET_VEHICLE_VARIABLE, libsumo::VAR_SPEED, vehicleID);
}

void TraCIAPI::VehicleScope::my_setSpeed(const std::string& vehicleID, int value) const {
    setSpeed(vehicleID, double(value));
}

void
TraCIAPI::VehicleScope::my_changeColor(const std::string& vehicleID) const {
    tcpip::Storage content;
    content.writeUnsignedByte(libsumo::TYPE_COLOR);
    content.writeUnsignedByte(255);
    content.writeUnsignedByte(0);
    content.writeUnsignedByte(0);
    content.writeUnsignedByte(255);
    ...
    myParent.processSet(libsumo::CMD_SET_VEHICLE_VARIABLE);
    tcpip::Storage inMsg;
    myParent.check_resultState(inMsg, libsumo::CMD_SET_VEHICLE_VARIABLE);
}

void
TraCIAPI::VehicleScope::my_changeLane(const std::string& vehicleID, char lane, int duration) const {
    tcpip::Storage content;
    content.writeUnsignedByte(libsumo::TYPE_COMPOUND);
    content.writeInt(2);
    content.writeUnsignedByte(libsumo::TYPE_BYTE);
...
std::string
TraCIAPI::VehicleScope::my_getLaneID(const std::string& vehicleID) const {
    return myParent.getString(libsumo::CMD_GET_VEHICLE_VARIABLE, libsumo::VAR_LANE_ID, vehicleID);
}

int
TraCIAPI::VehicleScope::my_getLaneIndex(const std::string& vehicleID) const {
    return myParent.getInt(libsumo::CMD_GET_VEHICLE_VARIABLE, libsumo::VAR_LANE_INDEX, vehicleID);
}
