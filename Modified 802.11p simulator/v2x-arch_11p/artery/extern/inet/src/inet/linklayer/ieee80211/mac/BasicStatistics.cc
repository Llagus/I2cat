...
//~ for obtaining mac address as string
//-------------------------------------
#include <sstream>
#include <iomanip> 
...
void BasicStatistics::frameReceived(Ieee80211Frame *frame)
{
    ...
    {
        // EV_WARN << "--> FRAME_OK, who_sent_it = " << utils->mac_who_sent(frame) << endl;
    }
...
//~ for distance errors
// --------------------
std::string BasicStatistics::stat_get_macbyte_str(Ieee80211Frame *frame, int idx)
{
    std::stringstream stream;
    stream << std::setfill('0') 
           << std::setw(2) 
           << std::hex 
           << int(utils->mac_who_sent(frame).getAddressByte(idx));
    return stream.str();
}

//~ for distance errors
//---------------------
std::string BasicStatistics::stat_get_txmac_str(Ieee80211Frame *frame)
{
    std::string tx_mac_str = stat_get_macbyte_str(frame, 0) +
         ":" + stat_get_macbyte_str(frame, 1) +
         ":" + stat_get_macbyte_str(frame, 2) +
         ":" + stat_get_macbyte_str(frame, 3) +
         ":" + stat_get_macbyte_str(frame, 4) +
         ":" + stat_get_macbyte_str(frame, 5);
    return tx_mac_str;
}
